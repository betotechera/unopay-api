...
res = conn.exec_params("""SELECT ci.installment_number, ci.expiration, c.begin_date, p.name, c.code, * from unovation.contract_installment ci 
...
res.each { |installment|
   expiration = DateTime.parse(installment['expiration'])
   installment_number = installment['installment_number']
   newExpiration = expiration << 1  
   puts '-----------------------------------------------------------------' if installment_number.eql? '1' 
   puts "#{installment_number} #{expiration.strftime("%d/%m/%Y")} >> #{newExpiration.strftime("%d/%m/%Y")} #{installment['name']} #{installment['code']}"
}
